...
def run(data_path, learner_type, fraction):
    ...
    predicted_target = test_targets
    if learner_type == 'prior_probability':
        naiveBayes = PriorProbability()
        naiveBayes.fit(train_features, train_targets)
        predicted_target = naiveBayes.predict(test_features)

    elif learner_type == 'decision_tree':
        decisionTree = DecisionTree(attribute_names)
        decisionTree.fit(train_features, train_targets)
        predicted_target = decisionTree.predict(test_features)

    # Order of these returns must be maintained
    cm = confusion_matrix(test_targets, predicted_target)
    acc = accuracy(test_targets, predicted_target)
    p, r = precision_and_recall(test_targets, predicted_target)
    f1 = f1_measure(test_targets, predicted_target)
