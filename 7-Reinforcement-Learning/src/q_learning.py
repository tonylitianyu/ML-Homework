...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        for i in range(steps):
            ...
            if prob < self._get_epsilon(i/steps):
                action = env.action_space.sample()
            else:
                curr_val = state_action_values[state]
                find_tie = np.flatnonzero(np.isclose(curr_val, curr_val.max()))
                action = np.random.choice(find_tie)

            new_state, r, done, _ = env.step(action)

            avg_r += r
            if (i+1) % s_avg_r == 0:
                rewards[int(i/s_avg_r)] = avg_r/s_avg_r
                avg_r = 0.0

            N[state][action] += 1
            alpha = 1/N[state][action]
            next_val = state_action_values[new_state]
            find_next_tie = np.flatnonzero(np.isclose(next_val, next_val.max()))
            next_action = np.random.choice(find_next_tie)


            state_action_values[state][action] += alpha*(r + self.discount*state_action_values[new_state][next_action] - state_action_values[state][action])
