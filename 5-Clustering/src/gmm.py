...
class GMM():
    ...
    def _e_step(self, features):
        ...
        """
        posterior = np.zeros((features.shape[0], self.n_clusters))

        for k in range(self.n_clusters):
            feature_gaussian = self._posterior(features, k)
            posterior[:,k] = feature_gaussian
    ...
    def _m_step(self, features, assignments):
        ...
        Arguments:
            ...
            assignments {np.ndarray} -- Soft assignments of each point to one of the cluster,
                given by _e_step. (shape is (features.shape[0], self.n_clusters))
        ...
        response = np.sum(assignments, axis=0)
        ...
        for k in range(0,self.n_clusters):
            ...
            for n in range(0,len(features)):
                #mean
                curr_mean_numer = assignments[n][k]*features[n]

                mean_numer_sum.append(curr_mean_numer)

                #covariance

                curr_cov_numer = assignments[n][k]*np.square(features[n] - self.means[k])
                

                cov_numer_sum.append(curr_cov_numer)

            mean_numer_sum = np.array(mean_numer_sum)
            self.means[k] = np.sum(mean_numer_sum,axis=0)/response[k]


            cov_numer_sum = np.array(cov_numer_sum)

            if self.covariance_type == 'diagonal':
                self.covariances[k] = np.sum(cov_numer_sum,axis=0)/response[k]


            if self.covariance_type == 'spherical':
                self.covariances[k] = np.mean(np.sum(cov_numer_sum, axis=0)/response[k])
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        """
        features_log_lh = np.zeros(len(features))
        for i in range(len(features)):
            single_logpdf = multivariate_normal.logpdf(features[i], self.means[k_idx], self.covariances[k_idx])
            final_log = np.log(self.mixing_weights[k_idx]) + single_logpdf
            features_log_lh[i] = final_log

        
        return features_log_lh
