...
class MultiClassGradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        Modifies:
            ...
            self.classes - (np.array) A numpy array of the unique target
                ...
                in predict.
        """
        self.classes, counts = np.unique(targets, return_counts=True)
        ...
        for c in self.classes:
            ...
            for t in range(len(temp_targets)):
                if temp_targets[t] == c:
                    temp_targets[t] = 1
                else:
                    temp_targets[t] = -1
            ...
            learner.fit(features, temp_targets, batch_size=batch_size, max_iter=max_iter)
    ...
    def predict(self, features):
        ...
        for i in range(len(predictions)):
            ...
            for m in range(0,len(self.model)):
                temp_confidence = self.model[m].confidence(features[i].reshape(1,-1))[0]

                if temp_confidence > max_confidence:
                    max_confidence = temp_confidence
                    max_confidence_idx = m


            predictions[i] = self.classes[max_confidence_idx]
...
#     features, _, targets, _ = load_data('blobs', base_folder='../data')
...
#     print(targets)
#     print(predictions)
#     print(np.all(predictions == targets))
